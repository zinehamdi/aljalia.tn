...
<div>
    ...
    <div class="py-4">
        @if(Auth::user()->city_id)
            <div class="px-4 mb-4 flex justify-between items-center bg-gray-50/50 p-2 rounded-xl mx-4 border border-gray-100"
                dir="{{ app()->getLocale() == 'ar' ? 'rtl' : 'ltr' }}">
                <div
                    class="text-sm font-bold text-gray-700 {{ app()->getLocale() == 'ar' ? 'font-arabic' : '' }} {{ app()->getLocale() == 'ar' ? 'text-right' : 'text-left' }}">
                    @if($currentCityOnly)
                        {{ __('Only posts from') }} {{ Auth::user()->city->name }}
                    @else
                        {{ __('Posts from all') }} {{ Auth::user()->country->name }}
                    @endif
                </div>
            ...
            @empty
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-8 text-center mt-4 mx-4">
                    ...
                    <h3 class="font-bold text-gray-700 {{ app()->getLocale() == 'ar' ? 'font-arabic' : '' }} mb-2">
                        {{ __('The light is out here...') }}
                    </h3>
                    <p class="text-gray-500 text-sm {{ app()->getLocale() == 'ar' ? 'font-arabic' : '' }} mb-4 opacity-80">
                        {{ __('There are no posts in this neighborhood yet. Be the first to post!') }}
                    </p>
                    <a href="{{ route('posts.create', ['category_id' => $category->id]) }}" wire:navigate
