...
new class extends Component {
    ...
    public $continents = [
        ...
        ['id' => 'gulf', 'name' => 'الخليج والشرق الأوسط', 'icon' => '🕌'],
        ['id' => 'north_america', 'name' => 'أمريكا الشمالية', 'icon' => '🏔️'],
    ];
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg border border-gray-100">
            <div class="p-8 text-gray-900 flex flex-col items-center">
                ...
                @if($step !== 'continent')
                    <div class="w-full flex justify-start mb-4">
                        <button wire:click="goBack"
                            class="text-gray-400 hover:text-aljalia-red flex items-center gap-1 text-sm font-arabic">
                ...
                <div class="text-center mb-10">
                    ...
                    <h1 class="text-3xl font-black text-gray-900 mb-2 font-arabic tracking-tight">وصلنا!</h1>
                    <p class="text-gray-500 font-arabic text-md">
                        @if($step === 'continent')
                            في أي قارة تعيش؟
                        @elseif($step === 'country')
                            اختر البلد
                        ...
                        @foreach($continents as $continent)
                            <button wire:key="continent-{{ $continent['id'] }}"
                                ...
                                <div class="flex items-center gap-4">
                                    ...
                                    <span class="font-bold text-gray-700 font-arabic text-lg">{{ $continent['name'] }}</span>
                                </div>
                    ...
                    <div class="grid grid-cols-2 gap-4 w-full">
                        @foreach($countries as $country)
                            <button wire:key="country-{{ $country->id }}" wire:click="selectCountry({{ $country->id }})"
                                ...
                                <span class="font-bold text-gray-800 font-arabic">{{ $country->name }}</span>
                            </button>
                        @endforeach
                    </div>
                @endif
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            @forelse($cities as $city)
                                <button wire:key="city-{{ $city->id }}" wire:click="selectCity({{ $city->id }})"
                                    class="p-4 border-2 border-gray-100 rounded-xl text-center font-bold font-arabic text-gray-700 hover:border-aljalia-red hover:bg-red-50 transition-all shadow-sm">
                                    {{ $city->name }}
                                </button>
                            @empty
                                <div class="col-span-2 text-center py-4 text-gray-400 font-arabic">لا توجد مدن مضافة حالياً.
                                </div>
                            @endforelse
                        </div>

                        <button wire:click="saveLocation"
                            class="w-full mt-6 py-4 bg-gray-100 text-gray-500 rounded-2xl font-bold font-arabic hover:bg-gray-200 transition-colors uppercase tracking-wider text-sm">
                            تخطي واختيار لاحقاً
                        </button>
                    </div>
                @endif
                ...
                <div class="mt-12 text-center">
                    <p class="text-[11px] text-gray-400 font-arabic">منصة الجالية.tn دليلك وعايلتك في الغربة</p>
                </div>
            </div>
        </div>
    </div>
