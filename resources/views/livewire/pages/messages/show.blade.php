...
<div class="flex flex-col h-[calc(100vh-140px)]"
    ...
    <x-slot name="header">
        <div class="flex items-center gap-3 w-full">
            ...
            <div class="text-right">
                ...
                @if($otherUser->city)
                    <p class="text-[11px] text-red-100 font-arabic truncate max-w-[150px] opacity-90">
    ...
    <div wire:poll.5s class="flex-1 overflow-y-auto px-4 py-4 space-y-3 bg-gray-50/50" id="chat-messages">
        @forelse($messages as $message)
            @if($message->sender_id == $currentUserId)
                <!-- My message -->
                <div wire:key="msg-{{ $message->id }}" class="flex justify-start">
                    <div
                        class="bg-aljalia-red text-white rounded-2xl rounded-tr-sm px-4 py-2.5 max-w-[80%] shadow-sm relative group">
                        <p class="text-sm font-arabic whitespace-pre-line leading-relaxed text-right" dir="rtl">
                            {{ $message->body }}</p>
                        <div class="text-[9px] text-red-100/80 mt-1 flex items-center justify-end gap-1">
            ...
            @else
                ...
                <div wire:key="msg-{{ $message->id }}" class="flex justify-end">
                    <div
                        ...
                        <p class="text-sm font-arabic whitespace-pre-line leading-relaxed text-right" dir="rtl">
                            {{ $message->body }}</p>
                        <div class="text-[9px] text-gray-400 mt-1 text-left">{{ $message->created_at->format('H:i') }}</div>
                    </div>
                </div>
            @endif
        @empty
            <div class="text-center py-20 bg-white rounded-2xl mx-2 border border-dashed border-gray-200 mt-4">
                ...
                <p class="text-gray-500 text-sm font-arabic px-6">ابدأ محادثة مع {{ $otherUser->name }}!</p>
            </div>
        @endforelse
    </div>
    ...
    <div
        ...
        <form wire:submit.prevent="sendMessage" class="flex gap-2 items-end max-w-lg mx-auto" dir="rtl">
            <textarea wire:model="newMessage" rows="1" placeholder="اكتب رسالتك..."
                class="flex-1 rounded-2xl border-gray-200 shadow-sm focus:border-aljalia-red focus:ring focus:ring-red-100 focus:ring-opacity-50 font-arabic text-sm px-4 py-3 bg-gray-50 resize-none max-h-32 text-right transition-all"
            ...
            <button type="submit"
                ...
                <svg wire:loading.remove wire:target="sendMessage" class="w-6 h-6 rotate-180" fill="none"
